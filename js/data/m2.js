...
{ id:"m2-mod5", title:"Labwork 2 — GotoXY e FollowLine no SimTwo (guiado)", minutes:60, kind:"labwork",
  ...
  { type:"labtask", title:"Sub-tarefa (1) — De (v, ω) a (v1, v2)",
    ...
    q:"Qual é o par correto, adotando a convenção do M1 (v1 = roda direita, ω>0 = rodar à esquerda)?",
    kind:"mcq",
    ...
    answer:0,
    hints:["Inverte as fórmulas do M1: v=(v1+v2)/2 e ω=(v1−v2)/b.","Soma e subtrai: v1 = v + ωb/2. Confirma: (v1+v2)/2 = v ✓ e (v1−v2)/b = ω ✓.","Pensa fisicamente: para rodar à esquerda, qual das rodas tem de andar mais depressa?"],
    solution:"De v=(v1+v2)/2 e ω=(v1−v2)/b resolve-se: <b>v1 = v + ω·b/2</b>, <b>v2 = v − ω·b/2</b>. Para ω>0 (rodar à esquerda) a roda direita acelera.<br><br><div class='hl'>⚠ <b>Atenção ao abrir o MotorVel() do SimTwo:</b> vais encontrar o sinal ao contrário —<br><code>v1 := vlin_prev - omega_prev*(WheelDist)/2.0;</code><br><code>v2 := vlin_prev + omega_prev*(WheelDist)/2.0;</code><br>Não está errado nem contradiz o de cima: neste robô o eixo 0 (<code>v1</code>) é a roda <b>esquerda</b> e o eixo 1 (<code>v2</code>) a <b>direita</b>. É a mesma física com os nomes trocados.<br>O que tem mesmo de bater certo é a coerência com a odometria: a roda que leva <code>+ω·b/2</code> aqui é a que entra com <b>+</b> no numerador do Δθ — e de facto o <code>predictPosition</code> desta lab usa <code>delta_theta := (odo2-odo1)*ToMetres/WheelDist</code>. ✓<br>Nota ainda que <code>SetAxisSpeedRef</code> recebe velocidade <b>angular</b> do eixo (rad/s), daí a divisão por <code>WheelRadius</code>, e que o <code>Accelerate()</code> é chamado antes — é ele que aplica as rampas DVMAX/DWMAX da tarefa (3), por isso o MotorVel usa <code>vlin_prev</code>/<code>omega_prev</code>.</div>" },
